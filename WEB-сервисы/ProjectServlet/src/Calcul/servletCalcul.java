...
public class servletCalcul extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        double num_1 = jsonObject.get("a").getAsDouble();
        double num_2 = jsonObject.get("b").getAsDouble();
        String action = jsonObject.get("math").getAsString();

        if ((action.equals("/") && num_2 == 0.0) || action == null) {
        	pw.print(gson.toJson("Невозможно выполнить операцию!"));
        } else if (action.equals("+")) {
    		pw.print(gson.toJson("result: " + (num_1 + num_2)));
    	} else if (action.equals("-")) { 
    		pw.print(gson.toJson("result: " + (num_1 - num_2)));
    	} else if (action.equals("*")) {
    		pw.print(gson.toJson("result: " + (num_1 * num_2)));
    		...
    		pw.print(gson.toJson("Введите корректные данные!"));
    	}
	}
}